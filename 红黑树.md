...
接下来我们就来看它是如何进行二叉化的。但在这之前，最好先观看我对 AVL 树以及 B 树的讲解,这样才能对红黑树有体系性的理解,而不至于只是记住一些规则。
...
插入54后，51这个节点属于两边失衡,进行了相应的旋转操作。旋转完之后重新变色,使他符合我们红黑树的要求。
红节点两边失衡(LL,RR)
...
插入22,违法了不红红,进行了相应的调整,实际对应的是中间失衡的旋转操作。对于杠杆21-23
...
但是红黑树不管如何推举，它肯定仍然是所有推举解集中的一种。之后在删除中，我们也会看到红黑树重新推举谁身不由己，但是它肯定是解集中的一种。
...
<video src="assets/rb-delete-case4-btree.webm" width="682" height="466" controls autoplay loop muted playsinline preload="auto" aria-label="删除单个黑节点（对应 B 树节点的红黑二叉编码）：25 与 10 折叠为同一成员层，25 变黑并将 17、28 变红；删除红色 28，保留黑色 25 和红色 17，整个过程不旋转"></video>
